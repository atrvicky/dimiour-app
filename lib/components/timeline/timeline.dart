...
import 'package:app/components/timeline/timeline_utils.dart';
...
/// Load all the resources from the local bundle.
  ...
  /// This function will load and decode `timline.json` from disk,
  /// decode the JSON file, and populate all the [TimelineEntry]s.
  Future<List<TimelineEntry>> loadFromNetwork(
      notification.UserTimeline userTimeline) async {
    List<TimelineEntry> allEntries = [];
    _backgroundColors = [];
    _tickColors = [];
    _headerColors = [];
    ...
    /// The JSON decode doesn't provide strong typing, so we'll iterate
    /// on the dynamic entries in the [jsonEntries] list.
    for (notification.Notification entry in userTimeline.notifications) {
      /// Sanity check.
      if (entry != null) {
        /// Create the current entry and fill in the current date if it's
        /// an `Incident`, or look for the `start` property if it's an `Era` instead.
        ...
        if ([
          ...
          NotificationType.WaterBreak,
          NotificationType.Posture
        ].contains(entry.type)) {
          timelineEntry.type = TimelineEntryType.Incident;
          dynamic date = entry.timeStamp;
          timelineEntry.start = date is int ? date.toDouble() : date;
        } else if ([
          NotificationType.WorkBreakShort,
          NotificationType.WorkBreakLong,
          NotificationType.FoodBreak
        ].contains(entry.type)) {
          timelineEntry.type = TimelineEntryType.Era;
          dynamic start = entry.timeStamp;
        ...
        timelineEntry.timeStamp = entry.createdTime;
        ...
        dynamic ticks = [
          ...
          NotificationType.Posture
            ...
            ? {
                "background": [113, 152, 188, 240],
                "long": [255, 255, 255, 75],
                "short": [255, 255, 255, 51],
                "text": [255, 255, 255, 157]
              }
            : {
                "background": [245, 245, 245, 240],
                "long": [0, 0, 0, 60],
                "short": [0, 0, 0, 35],
                "text": [0, 0, 0, 110]
              };
        ...
        if ([
          ...
          dynamic end = entry.type == NotificationType.FoodBreak
              ? entry.timeStamp + 40000
              : entry.type == NotificationType.WorkBreakShort
                  ? entry.timeStamp + 30000
                  : entry.timeStamp + 30000;
          timelineEntry.end = end is int ? end.toDouble() : end;
        } else if (timelineEntry.type == TimelineEntryType.Era) {
          timelineEntry.end = DateTime.now().year.toDouble() * 10.0;
        ...
        /// The label is a brief description for the current entry.
        timelineEntry.label = entry.type == NotificationType.WaterBreak
            ? 'Water Break'
            : entry.type == NotificationType.FoodBreak
                ? 'Food break'
                : entry.type == NotificationType.WorkBreakShort
                    ? 'Short Break'
                    : entry.type == NotificationType.WorkBreakLong
                        ? 'Long break'
                        : entry.type == NotificationType.Posture
                            ? 'Fix Posture'
                            : 'Motivate';
